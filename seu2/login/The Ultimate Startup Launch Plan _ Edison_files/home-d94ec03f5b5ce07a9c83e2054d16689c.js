...
(function() {
  var email_uniqueness, password_length, re, sign_up_form_btn_check, unique_email, valid_email_format, verify_email;

  valid_email_format = void 0;

  unique_email = void 0;

  password_length = void 0;

  re = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;

  email_uniqueness = function(email) {
    $.post('/email_validate', {
      email: email
    }, function(data) {
      if (data.status) {
        unique_email = true;
        sign_up_form_btn_check();
      } else {
        $('#user_email_error').html('<div class="user-field-error">The email you entered is already taken</div>');
        unique_email = false;
        sign_up_form_btn_check();
      }
    });
  ...
  sign_up_form_btn_check = function() {
    if (valid_email_format && unique_email && password_length) {
  ...
  verify_email = function(email) {
    ...
    $.post('/email_validate', {
      ...
      if (data.status === 'email available') {
        $('#email-reset-pw-errors').html('<div class="user-field-error">The email you entered was not found</div>');
      } else {
        $form.get(0).submit();
      }
    });
  ...
  $(document).on('blur', '#user_email', function() {
    var email, test_result;
    email = $(this).val();
    if (email !== '') {
      test_result = re.test(email);
      if (test_result) {
        $('#user_email_error').empty();
        valid_email_format = true;
        return email_uniqueness(email);
      } else {
        $('#user_email_error').html('<div class="user-field-error">Your email is invalid</div>');
        valid_email_format = false;
        return sign_up_form_btn_check();
      }
    }
  });

  $(document).on('keypress keydown keyup', '#user_password', function() {
    var pw, pw_length;
    pw = $(this).val();
    if (pw !== '') {
      pw_length = pw.length;
  ...
  $(document).on('submit', '.reset-pw-form', function(event) {
    var email;
  ...
  $(document).on('keypress keydown keyup', '#external-feedback-form', function() {
    var email, email_cond, msg, msg_cond, save_btn;
